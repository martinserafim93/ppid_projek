...
<div class="card shadow-sm border-0 mb-4">
    <div class="card-header bg-white py-3 d-flex flex-row align-items-center justify-content-between border-bottom">
        <h6 class="m-0 font-weight-bold text-dark">Daftar Admin & Pimpinan</h6>
        
        <form action="" method="get" class="d-flex">
            <select name="role" class="form-select form-select-sm border-secondary border-opacity-50" onchange="this.form.submit()">
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="small text-muted">Bergabung: <?= date('d M Y', strtotime($user['created_at'])) ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    ...
                                    <?php if (!empty($user['phone'])) : ?>
                                        <div class="text-dark small mt-1"><i class="bi bi-telephone text-muted me-1"></i> <?= esc($user['phone']) ?></div>
                                    <?php endif; ?>
                                </td>
                                <td class="text-center">
                                    ...
                                    <?php else : ?>
                                        <span class="badge bg-info text-dark px-3 py-2">Pimpinan</span>
                                    <?php endif; ?>
                                </td>
                                <td class="text-center">
                                    <div class="form-check form-switch d-flex justify-content-center m-0">
                                        <input class="form-check-input toggle-status" type="checkbox" role="switch" 
                                            ...
                                            <?= $isSelf ? 'disabled title="Tidak bisa menonaktifkan akun sendiri"' : '' ?>>
                                    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body text-center p-4">
                <p class="mb-2 text-muted">Password baru untuk pengguna <strong id="reset-user-name" class="text-dark"></strong>:</p>
                ...
                <div class="alert alert-warning mb-0 border-warning border-opacity-25" role="alert">
                    ...
                    <strong>Penting!</strong> Copy dan simpan password ini sekarang. Password ini tidak akan ditampilkan lagi setelah jendela ditutup.
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    toggleSwitches.forEach(toggle => {
        toggle.addEventListener('change', function() {
            ...
            Swal.fire({
                title: isChecked ? 'Aktifkan Akun?' : 'Nonaktifkan Akun?',
                text: isChecked ? 'Pengguna akan dapat login kembali.' : 'Pengguna tidak akan dapat login ke sistem.',
                icon: 'warning',
    ...
    resetButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                html: `Anda akan mereset password untuk pengguna <strong>${name}</strong>.<br><br>Sistem akan membuatkan password acak baru.`,
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`<?= base_url('admin/users/reset-password/') ?>${id}`, {
                        method: 'POST',
                        headers: {
    ...
    copyBtn.addEventListener('click', function() {
        ...
        navigator.clipboard.writeText(passwordToCopy).then(function() {
            ...
            Swal.fire('Ops!', 'Gagal mengcopy password. Silakan blok dan copy manual.', 'warning');
        });
    });
});
</script>
